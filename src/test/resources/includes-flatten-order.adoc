...
:imagesdir: https://s3.amazonaws.com/dev.assets.neo4j.com/course/graph-data-modeling/images/
...
. What information can be attached to a node?
Which of these are required?
.. Labels & properties.
None required
. What information can be attached to a relationship?
Which of these are required?
.. Direction, type, and properties.
Direction and type are required.
...
.. Neo4j will predict which of the node sets in the query is smallest.
It will begin there.
In this case, r is likely to be smallest, as it has a property filter.
...
[NOTE]
Every organization has their own standards for how requirements are documented.
You will not be learning how to document requirements, but rather focus on the model itself.
--
...
The data is typically the "things" described in the use cases.
You don't include "system" or "user" as part of the data.
--
...
* In Neo4j Enterprise Edition, there is no limit to the number entities (nodes) in the graph.
(Community Edition has a limit of 34B)
--
...
Direct them to find labels first.
Then, go through the list of entities and ask for identifier and question-answering properties for each.
If someone proposes a decorator, note it as such, and do not keep it.

If anyone start thinking about connections, remind them to stay narrowly focused on entities.
Reinforce the importance of staying disciplined.
...
. What episode follows The Ark in Space?
(NEXT)
. What episode came before Genesis of the Daleks?
(PREVIOUS)
...
Remind them what was discussed in the Index-Free Adjacency module of the Intro course: in a graph database, it is generally better to have lots of small objects than a few large objects.
Fanout like this is emblematic of that.
--
...
Should these qualifiers be types or properties?
Why?
--
